...
const DFSPage = () => {
  ...
  const buildAdjList = () => {
    const adj = {};
    for (let item of nodes) { const node = item.node.trim(); if (!node) continue; adj[node] = item.neighbors.split(",").map((n) => n.trim()).filter(Boolean); }
    return adj;
  };
  ...
  const handleReplay = () => { clearTimeout(timerRef.current); setIsPlaying(false); setSteps([]); setVisited(new Set()); setCurrentStepIndex(0); setExplanation(""); };
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      setExplanation(step.found ? `🎯 Target node ${target} found!` : `Visiting node ${step.current}`);
  ...
  return (
    <div className="min-h-screen pt-24 sm:pt-32 pb-16 px-3 sm:px-6" style={{ color: 'hsl(0 0% 96%)' }}>
      ...
      <div className="algo-split-layout">
            ...
            <div className="flex justify-center mb-4">
              <svg viewBox="0 0 520 440" className="responsive-svg rounded-xl" style={{ background: 'hsl(220 20% 6%)', maxHeight: '440px' }}>
                ...
                {nodeKeys.map((node) => {
                  ...
                  return (<g key={node}><circle cx={positions[node].x} cy={positions[node].y} r="22" fill={color} style={{ filter: node === currentNode ? 'drop-shadow(0 0 8px hsl(40 90% 55%))' : 'none' }} /><text x={positions[node].x} y={positions[node].y + 5} textAnchor="middle" fill="hsl(220 20% 6%)" fontWeight="bold" fontSize="14">{node}</text></g>);
                })}
              </svg>
            </div>
        ...
        <div className="algo-code-panel"><CodeViewer code={DFS_CODE} highlightedLine={highlightedLine} title="dfs.js" /></div>
      </div>
    </div>
  );
};
