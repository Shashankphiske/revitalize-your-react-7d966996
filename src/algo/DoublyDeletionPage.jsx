...
const DoublyDeletionPage = () => {
  ...
  const fetchSteps = async (arr, index) => { const res = await fetch("http://localhost:3000/linkedlist/doublydeletion", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ arr, index: parseInt(index) }) }); return (await res.json()).steps; };
  ...
  return (
    <div className="min-h-screen pt-32 pb-16 px-6" style={{ color: 'hsl(0 0% 96%)' }}>
      <AlgoPageHeader icon="✂️" title="Doubly Linked List – Deletion" description="Deletion adjusts both prev and next pointers to remove a node." complexity={{ time: "O(n)", space: "O(1)" }} />
