...
const getHighlightedLine = (step) => {
  if (!step) return null;
  if (step.action === "delete-complete") return 10;
  ...
  const handlePlay = async () => {
    ...
    try {
      if (steps.length === 0) {
        ...
        setExplanation("Starting deletion in doubly linked list...");

        const fetchedSteps = await fetchSteps(arr, index);
    ...
    if (currentStepIndex < steps.length) {
      ...
      timerRef.current = setTimeout(() => {
        ...
        if (step.current !== undefined) {
          setExplanation(`Deleting node at index ${deleteIndex}...`);
        } else {
          setExplanation("Deletion completed successfully.");
        }

        setCurrentStepIndex((prev) => prev + 1);
      }, 1500);
    } else {
      setExplanation("✅ Doubly Linked List deletion complete");
      setIsPlaying(false);
    }
  ...
  return (
    ...
    >
      <AlgoPageHeader
        ...
        description="Deletion adjusts both prev and next pointers to remove a node."
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div>
              ...
              <input
                ...
                disabled={isPlaying}
                className="px-4 py-3 rounded-xl w-32 outline-none"
                style={inputStyle}
              />
            </div>
          </div>
