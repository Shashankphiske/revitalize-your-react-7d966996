...
const DoublyReversalPage = () => {
  ...
  const fetchSteps = async (arr) => { const res = await fetch("http://localhost:3000/linkedlist/doublyreversal", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ arr }) }); return (await res.json()).steps; };
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      setExplanation(step.current ? `Swapping pointers. Current: ${step.current}` : "Reversal complete!");
  ...
  return (
    <div className="min-h-screen pt-32 pb-16 px-6" style={{ color: 'hsl(0 0% 96%)' }}>
      <AlgoPageHeader icon="🔄" title="Doubly Linked List – Reversal" description="Reversal swaps prev and next pointers for each node." complexity={{ time: "O(n)", space: "O(1)" }} />
