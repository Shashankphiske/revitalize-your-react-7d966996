...
const getHighlightedLine = (step) => {
  if (!step) return null;
  // Highlight the visit(root) line at line 7 when touching a node
  return 7;
};
...
const PostorderPage = () => {
  ...
  const fetchSteps = async (adj) => {
    const res = await fetch(
      "http://localhost:3000/treealgo/postorder",
      {
        method: "POST",
    ...
    <div className="min-h-screen pt-24 sm:pt-32 pb-16 px-4 sm:px-6 text-white">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <AlgoPageHeader
          ...
          description="Visits nodes in Left → Right → Root order. Useful for tree deletion and expression evaluation."
