...
const SelectionSortPage = () => {
  ...
  const generateExplanation = (step) => {
    if (!step.comparing || step.comparing.length === 0) return "Selection Sort completed.";
    if (step.swapped) return `Swapped elements to place minimum in correct position.`;
    return `Comparing elements to find the minimum in unsorted region.`;
  };
  ...
  useEffect(() => {
    if (!isPlaying || currentStepIndex >= steps.length) {
      if (isPlaying && currentStepIndex >= steps.length && steps.length > 0) {
        ...
        setExplanation("Selection Sort completed. The array is now sorted.");
        setIsPlaying(false);
      }
      return;
    }
    ...
    <div className="min-h-screen text-white pt-24 pb-16 px-6">
      <div className="text-center max-w-4xl mx-auto mb-8">
        <div className="inline-block mb-4">
          <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center glow">
            <span className="text-2xl">🔍</span>
          </div>
        </div>
        <h1 className="text-4xl md:text-5xl font-bold mb-4">
          <span className="gradient-text">Selection Sort</span>
        </h1>
        <div className="glass-card rounded-2xl p-6 mb-6 text-left">
          <h2 className="text-xl font-semibold mb-3 gradient-text-secondary">About Selection Sort</h2>
          <p className="text-gray-300 text-sm mb-3 leading-relaxed">
            Selection Sort repeatedly selects the minimum element from the unsorted part of the array
            and swaps it with the first unsorted element. The sorted portion grows from left to right.
          </p>
          <div className="flex flex-wrap gap-4 mt-4">
            ...
            <div className="glass px-4 py-2 rounded-lg">
              ...
              <div className="text-red-400 font-semibold">Unstable</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="algo-split-layout">
        <div className="algo-visualization-panel">
          ...
          <div className="flex justify-center gap-6 mt-4 text-sm text-gray-300">
            <div className="flex items-center gap-2"><span className="w-4 h-4 bg-blue-500 rounded"></span> Unsorted</div>
            <div className="flex items-center gap-2"><span className="w-4 h-4 bg-purple-500 rounded"></span> Selected Min</div>
            <div className="flex items-center gap-2"><span className="w-4 h-4 bg-yellow-400 rounded"></span> Comparing</div>
            <div className="flex items-center gap-2"><span className="w-4 h-4 bg-red-500 rounded"></span> Swapping</div>
          </div>
        </div>
        <div className="algo-code-panel">
          <CodeViewer code={SELECTION_SORT_CODE} highlightedLine={highlightedLine} title="selection-sort.js" />
        </div>
      </div>
    </div>
  );
