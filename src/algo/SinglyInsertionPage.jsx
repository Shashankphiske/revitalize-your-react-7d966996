...
const CODE = ["function insert(head, value, pos) {", "  const newNode = new Node(value);", "  if (pos === 0) {", "    newNode.next = head;", "    return newNode;", "  }", "  let curr = head;", "  for (let i = 0; i < pos - 1; i++)", "    curr = curr.next;", "  newNode.next = curr.next;", "  curr.next = newNode;", "  return head;", "}"];
const getHighlightedLine = (step) => { if (!step) return null; if (step.action === "insert-complete") return 10; if (step.current !== null && step.current !== undefined) return 8; return null; };
...
const SinglyInsertionPage = () => {
  ...
  const fetchSteps = async (arr, value, index) => { const res = await fetch("http://localhost:3000/linkedlist/singlyinsertion", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ arr, value: parseInt(value), index: parseInt(index) }) }); return (await res.json()).steps; };
  ...
  const handlePlay = async () => {
    ...
    if (steps.length === 0) {
      ...
      if (isNaN(Number(insertValue))) { setError("Invalid value!"); return; }
      setError(""); setCurrentStepIndex(0); setExplanation("Starting Insertion...");
  ...
  return (
    <div className="min-h-screen pt-32 pb-16 px-6" style={{ color: 'hsl(0 0% 96%)' }}>
      <AlgoPageHeader icon="🔗" title="Singly Linked List – Insertion" description="Insertion involves creating a new node and adjusting pointers to insert at the desired position." complexity={{ time: "O(n)", space: "O(1)" }} />
      ...
      <div className="algo-split-layout">
              ...
              {currentStep.list && currentStep.list.map((value, idx) => {
                ...
                return (
                  <React.Fragment key={idx}>
                    <div className="w-20 h-14 flex items-center justify-center text-xl font-bold rounded-xl transition-all duration-500" style={{ background: bg, color: 'hsl(220 20% 6%)', transform: scale, boxShadow: shadow }}>{value}</div>
                    {idx < currentStep.list.length - 1 && <span className="text-2xl" style={{ color: 'hsl(220 10% 40%)' }}>→</span>}
                  </React.Fragment>
                );
              })}
            </div>
        ...
        <div className="algo-code-panel"><CodeViewer code={CODE} highlightedLine={highlightedLine} title="singly-insertion.js" /></div>
      </div>
    </div>
  );
};
