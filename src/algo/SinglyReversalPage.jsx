...
const CODE = ["function reverse(head) {", "  let prev = null, curr = head;", "  while (curr !== null) {", "    let next = curr.next;", "    curr.next = prev;", "    prev = curr;", "    curr = next;", "  }", "  return prev;", "}"];
const getHighlightedLine = (step) => { if (!step) return null; if (step.current !== null && step.current !== undefined) return 4; return 8; };
...
const SinglyReversalPage = () => {
  ...
  const fetchSteps = async (arr) => { const res = await fetch("http://localhost:3000/linkedlist/singlyreversal", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ arr }) }); return (await res.json()).steps; };
  ...
  const handlePlay = async () => {
    ...
    if (steps.length === 0) {
      ...
      setError(""); setCurrentStepIndex(0); setExplanation("Starting Reversal...");
      setSteps(await fetchSteps(arr));
    }
    setIsPlaying(true);
  };
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      setExplanation(step.current ? `Reversing: curr=${step.current}, prev=${step.prev || "null"}` : "Reversal complete!");
  ...
  return (
    <div className="min-h-screen pt-32 pb-16 px-6" style={{ color: 'hsl(0 0% 96%)' }}>
      <AlgoPageHeader icon="🔄" title="Singly Linked List – Reversal" description="Reversal changes the direction of all next pointers in the linked list." complexity={{ time: "O(n)", space: "O(1)" }} />
      <div className="max-w-5xl mx-auto mb-8">
        <div className="card rounded-2xl p-6">
          <label className="text-sm mb-2 block" style={{ color: 'hsl(220 10% 50%)' }}>List</label>
          <input value={initialList} onChange={(e) => setInitialList(e.target.value)} disabled={isPlaying} className="w-full px-4 py-3 rounded-xl outline-none" style={inputStyle} />
      ...
      <div className="algo-split-layout">
          ...
          <AlgoVisualizationContainer>
            <div className="flex items-center justify-center gap-2 flex-wrap">
              {currentStep.list && currentStep.list.map((value, idx) => {
                ...
                else if (currentStep.prev === value) { bg = "hsl(262 80% 55%)"; scale = "scale(1.1)"; shadow = "0 0 15px hsl(262 80% 55% / 0.5)"; }
        ...
        <div className="algo-code-panel"><CodeViewer code={CODE} highlightedLine={highlightedLine} title="singly-reversal.js" /></div>
      </div>
    </div>
  );
};
