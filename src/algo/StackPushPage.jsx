...
const StackPushPage = () => {
  ...
  const fetchSteps = async (stack, toPush) => {
    const res = await fetch(
      "http://localhost:3000/stackalgo/stackpush",
      {
        method: "POST",
  ...
  const handlePlay = async () => {
    ...
    try {
      ...
      setExplanation("Starting Stack Push...");

      const data = await fetchSteps(stack, toPush);
      setSteps(data);
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      setExplanation(
        step.action === "push-start"
          ? `Pushing ${step.pointer?.current}...`
          : `Pushed ${step.pointer?.next}!`
      );
    ...
    <div className="min-h-screen pt-24 sm:pt-32 pb-16 px-4 sm:px-6 text-white">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <AlgoPageHeader
          icon="📥"
          title="Stack Push"
          description="Push adds an element to the top of the stack (LIFO – Last In First Out)."
          complexity={{
            time: "O(1)",
            space: "O(1)",
          }}
