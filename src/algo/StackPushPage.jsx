...
const STACK_PUSH_CODE = ["function push(stack, value) {", "  stack[++stack.top] = value;", "  return stack;", "}"];
const getHighlightedLine = (step) => { if (!step) return null; if (step.action === "push-start") return 1; if (step.action === "push-complete") return 2; return null; };
...
const StackPushPage = () => {
  ...
  const fetchSteps = async (stack, toPush) => { const res = await fetch("http://localhost:3000/stackalgo/stackpush", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ stack, push: toPush }) }); return (await res.json()).steps; };
  ...
  const handlePlay = async () => {
    ...
    if (steps.length === 0) {
      ...
      if (toPush.length === 0) { setError("Invalid push values!"); return; }
      setError(""); setCurrentStepIndex(0); setExplanation("Starting Stack Push...");
      setSteps(await fetchSteps(stack, toPush));
    }
    setIsPlaying(true);
  };
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      setExplanation(step.action === "push-start" ? `Pushing ${step.pointer.current}...` : `Pushed ${step.pointer.next}!`);
  ...
  return (
    <div className="min-h-screen pt-32 pb-16 px-6" style={{ color: 'hsl(0 0% 96%)' }}>
      <AlgoPageHeader icon="📥" title="Stack Push" description="Push adds an element to the top of the stack (LIFO – Last In First Out)." complexity={{ time: "O(1)", space: "O(1)" }} />
      <div className="max-w-5xl mx-auto mb-8">
        <div className="card rounded-2xl p-6">
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="flex-1"><label className="text-sm mb-2 block" style={{ color: 'hsl(220 10% 50%)' }}>Values to Push</label><input value={pushValues} onChange={(e) => setPushValues(e.target.value)} disabled={isPlaying} className="w-full px-4 py-3 rounded-xl outline-none" style={inputStyle} /></div>
          </div>
      ...
      <div className="algo-split-layout">
            ...
            <div className="flex flex-col-reverse items-center gap-2">
              {currentStep.list && currentStep.list.map((value, idx) => {
                ...
                if (currentStep.highlight && currentStep.highlight.includes(value)) {
                  if (currentStep.action === "push-complete") { bg = "hsl(145 65% 48%)"; scale = "scale(1.1)"; shadow = "0 0 15px hsl(145 65% 48% / 0.5)"; }
        ...
        <div className="algo-code-panel"><CodeViewer code={STACK_PUSH_CODE} highlightedLine={highlightedLine} title="stack-push.js" /></div>
      </div>
    </div>
  );
};
